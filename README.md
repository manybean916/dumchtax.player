...
- [사용 맥락 (USAGE)](docs/USAGE.md) — 서비스 정의·대상·사용자 여정·폴백
- [디자인 시스템 (DESIGN_SYSTEM)](docs/DESIGN_SYSTEM.md) — 컬러/타이포/스페이싱 토큰·컴포넌트·모션 규격
- [배포 & 키 등록 (DEPLOY)](docs/DEPLOY.md) — Netlify 서버리스 프록시로 키 숨기기 · 방문자 키 없이 실행

## 키 없이 실행 (호스팅 배포)

Netlify에 배포하고 서버 환경변수(`GEMINI_API_KEY`, `YOUTUBE_API_KEY`)를 등록하면, 방문자는 **키 입력 없이 배포 링크만으로** 바로 사용할 수 있습니다. 키는 서버리스 프록시(`/api/*`) 뒤에 숨겨져 브라우저·저장소에 노출되지 않습니다. 자세한 절차는 [DEPLOY.md](docs/DEPLOY.md) 참고.

> ⚠️ 이 앱은 백엔드 없는 정적 웹앱이라, 브라우저에 직접 넣은 키는 공개됩니다. 실제 키는 **깃헙·코드에 커밋하지 말고** Netlify 환경변수에만 등록하세요.
